...
void MatchingEngine::match_limit(Order* taker) {
    ...
    // Walk levels until no more crossing prices or taker is fully filled.
    while (to_uint(taker->remaining_quantity) > 0 && !opposite.empty()) {
        // Best opposing price: ask = begin (ascending), bid = rbegin (descending).
        ...
        // Fill orders FIFO within this level.
        ...
        // Level may now be empty; std::map will have removed it in cancel_order.
    }
...
void MatchingEngine::execute_fill(Order* maker, Order* taker, Quantity qty,
    ...
    // Always update the level aggregate atomically with remaining_quantity via
    // adjust_quantity.  For a full fill new_maker_qty == 0, so after this call
    // cancel_order → PriceLevel::remove will subtract 0 from the aggregate
    // (a no-op), which is correct because the aggregate was already decremented
    // here.
